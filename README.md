...
`BindingFlags.Public` 추가 후 라이드 로그에서 `[NvgAoDiag]` 경고가 사라졌습니다.
좋은 신호지만 **증거는 아닙니다.** 이 코드에서 경고가 안 뜨는 경우가 셋이었습니다:

1. 필드를 찾았다 (고쳐짐)
2. `_camera`에 `NightVision` 컴포넌트가 없다 (가드 여전히 꺼짐, 조용함)
3. `_hbao`/`_camera`가 null이라 검색까지 가지도 못했다 (조용함)

셋 다 로그가 똑같이 아무 말도 안 했습니다. 이번 4.1 작업 내내 물린 게 정확히 이
패턴이라 — 없는 기능은 자기가 없다고 말하지 않는다 — 세 경우를 갈라놨습니다:

- 필드를 찾으면 **찾았다고 찍습니다** (이름 + public/private까지). 성공도 로그를
  남겨야 침묵이 "확인됨"이 아니라 "코드가 안 돌았음"을 뜻하게 됩니다
- 컴포넌트 자체가 없으면 그것도 따로 경고합니다. 필드가 없는 것과 원인이 다르고
  대응도 다릅니다
- 필드만 못 찾으면 기존대로 bool 필드 후보 목록을 같이 찍습니다
