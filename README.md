...
**(같은 날 — 나이트비전 AO 가드: 원인 확정, 리플렉션 제거)**
...
"필드가 없다"와 로그상 완전히 똑같았습니다. `Public` 플래그 추가가 정확한 수정이었고,
경고가 사라진 것도 진짜 성공이었습니다.

그런데 public이면 **찾을 이유 자체가 없습니다.** 리플렉션을 걷어내고 직접 읽습니다:

```csharp
var nvOn = _nightVision != null && _nightVision._on;
```

이게 단순히 짧아서가 아닙니다. 리플렉션은 **런타임에 조용히 실패**하고 폴백으로
넘어가는데, 그 폴백이 하필 "수정 전 상태"라서 버그가 돌아와도 아무도 모릅니다.
직접 접근은 BSG가 이걸 다시 private으로 돌리면 **빌드 머신에서 컴파일 에러**가 납니다.
4.1 역난독화가 주는 이점이 정확히 이거고, 이번 포팅 내내 배운 것도 이거였습니다.

`On` 프로퍼티도 있지만 `_on`을 씁니다 — 09/03에 N키 양방향 추적을 실제로 검증한 게
`_on`이고, `On`은 자체 게터라 같은 값이라는 보장이 없습니다.

컴포넌트 자체를 못 찾는 경우(원인이 다르고 대응도 다름)는 여전히 경고를 남깁니다.
