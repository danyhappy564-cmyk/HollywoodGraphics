...
전부 **참조하던 `Assembly-CSharp.dll`이 SPT가 손대지 않은 BSG 원본**이었기 때문입니다.
개명 자체는 맞았습니다.

`tools/AssemblyDump`(HollywoodFX 레포)로 덤프해 보니 타입 15,137개 중 SPT 이름이
4.0 것도 4.1 것도 **0개**였고, 멤버 이름 상당수가 출력 불가능한 유니코드였습니다.

원인은 csproj의 탐색 순서였습니다 — 루트를 먼저 봤는데 4.1은 게임이 `SPT_Runtime\`
아래 있고, 루트에 손대지 않은 사본이 남아 있으면 그쪽이 이깁니다. 그리고 조용히
이깁니다: BSG 원본 이름으로 컴파일되니 SPT가 바꾼 건 전부 "찾을 수 없음"이 되고
마이그레이션 표가 틀린 것처럼 보입니다.

`SPT_Runtime\` → `SPT\` → 루트 순으로 고치고 네 가지 배치로 검증했습니다.
